...
class Form_App_Setting_Customer_Crossfit extends MY_Form
{
    //PERSON
    public $id_person;
    public $name;
    ...
    public function __construct( $isReadPost=FALSE )
    {
        ...
        //PROFILE
        $this->id_profile           = 2;//PERFIL DE CUSTOMER
        $this->isActive             = 1;

        //COMPANY_BRANCH
        $this->id_company_branchs   = NULL;
        
        $this->path_picture         = 'resources/img/nologo.png';
    ...
    public function getUserEntity()
    {
        ...
        //$eUser->password    = trim($this->username);
        /*if(!empty($this->password_new_repeat))
        {
            $eUser->password    = md5($this->password_new_repeat);
        }*/
    ...
    {
        $eCustomer = new eCustomer(FALSE);
        
        $eCustomer->id                  = empty($this->id_customer) ? 0 : $this->id_customer ;
        $eCustomer->id_person           = $this->id_person;
        $eCustomer->code                = trim($this->code);
        $eCustomer->registration_date   = $this->registration_date;
        
        return $eCustomer;
    }
    
    public function setCustomer(eCustomer $eCustomer)
    {
        $this->id_customer          = empty($eCustomer->id) ? 0 : $eCustomer->id ;
        $this->id_person            = $eCustomer->id_person;
        $this->code                 = $eCustomer->code;
        $this->registration_date    = $eCustomer->registration_date;
    }
    
    public function setPath( $uri )
    {
        $this->path_picture = $uri;
    }
