var MY_Layout_Controller = {
    ...
    _init: function ()
    {
        ...
        $('[action="redict-page-block"]').attr({
            'href': 'javascript:void(0);'
        }).click(function () {
            var html = "<div class='alert alert-warning fade in block-inner'><ul><li>Presione --Si-- cuando guardo toda la información</li><li>Presione --No-- cuando falta por guardar información</li></ul>'</div> <div class='has-warning'>¿Desea continuar con el cierre temporal de sesión?</div> ";
            //href="<?php echo site_url('app/block'); ?>"
            //Core.Alert.question('¿Esta seguro de cerrar temporalmente la sesión de usuario? <br /> Si tiene información por guardar, Guarde por favor!', function () = {$(this).attr('href',page);})
            Core.Alert.question(html, function () {
                location.href = Core.site_url('app/block');
            });
        });
        ...
        $('[action="action-change-profile"]').attr({
            'href': 'javascript:void(0);'
        }).click(function () {
            var _value = $(this).attr('_value');
            self.updatePerfilSession(_value);
        });

        $('[action="action-change-sucursal"]').attr({
            'href': 'javascript:void(0);'
        }).click(function () {
            var _value = $(this).attr('_value');
            self.updateSedeSession(_value);
        });
...
var Modal_My_Data = {
    ...
    formSetData: function (res)
    {
        ...
        if (res != null)
        {
            if (res.forms.module.password_current) {
                password_current = res.forms.module.password_current
            }
            ;
            if (res.forms.module.password_new) {
                password_new = res.forms.module.password_new
            }
            ;
            if (res.forms.module.password_new_repeat) {
                password_new_repeat = res.forms.module.password_new_repeat
            }
            ;
        }
