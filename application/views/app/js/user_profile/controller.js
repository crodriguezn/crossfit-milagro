...
var Profile_Controller = {
            ...
            if (res.data.eCatalogs['TIPO_IDENT'])
            {
                ...
                if (TIPO_IDENT.length > 0)
                {
                    ...
                    $.each(TIPO_IDENT, function (idx, tipo_identificacion)
                    {
                        ...
                        {
                            if (tipo_identificacion.value == UserProfile_Base.profile_form_default.tipo_identificacion.value)
                            {
            ...
            if (res.data.eCatalogs['GENDER'])
            {
                ...
                if (GENDER.length > 0)
                {
                    ...
                    $.each(GENDER, function (idx, genero)
                    {
                        if (typeof _value == 'undefined')
                        {
                            if (genero.value == UserProfile_Base.profile_form_default.gender.value)
                            {
                                isView = genero.value;
                            }
                        }
            ...
            if (res.data.eCatalogs['ESTADO_CIVIL'])
            {
                ...
                if (ESTADO_CIVIL.length > 0)
                {
                    ...
                    $.each(ESTADO_CIVIL, function (idx, estado_civil)
                    {
                        if (typeof _value == 'undefined')
                        {
                            if (estado_civil.value == UserProfile_Base.profile_form_default.estado_civil.value)
                            {
            ...
            if (res.data.eCatalogs['TIPO_DE_SANGRE'])
            {
                ...
                if (TIPO_DE_SANGRE.length > 0)
                {
                    ...
                    $.each(TIPO_DE_SANGRE, function (idx, tipo_sangre)
                    {
                        if (typeof _value == 'undefined')
                        {
                            if (tipo_sangre.value == UserProfile_Base.profile_form_default.tipo_sangre.value)
                            {
        ...
        UProfile_Model.loadPais(function (res) {
            ...
            if (res.data['cbo-pais'].length > 0)
            {
                ...
                $.each(res.data['cbo-pais'], function (idx, pais)
                {
                    if (typeof _value == 'undefined')
                    {
                        if (pais.value == UserProfile_Base.profile_form_default.id_pais.value)
                        {
                            isView = pais.value;
                        }
                    }
        ...
        UProfile_Model.loadProvincia(id_pais, function (res) {
            ...
            if (res.data['cbo-provincia'].length > 0)
            {
                ...
                $.each(res.data['cbo-provincia'], function (idx, provincia)
                {
                    if (typeof _value == 'undefined')
                    {
                        if (provincia.value == UserProfile_Base.profile_form_default.id_provincia.value)
                        {
            ...
            if (res.data['cbo-ciudad'].length > 0)
            {
                ...
                $.each(res.data['cbo-ciudad'], function (idx, ciudad)
                {
                    if (typeof _value == 'undefined')
                            //if (_value == 0)
                            {
                                if (ciudad.value == UserProfile_Base.profile_form_default.id_ciudad.value)
                                {
                                    isView = ciudad.value;
                                }
                            }
    ...
    {
        this.data(UserProfile_Base.profile_form_default);
    },
    dataLoad: function ()
    {
        var self = this;
    ...
    data: function (profile_data/*=UNDEFINED*/)
    {
        ...
        if (profile_data)
        {
            var name = profile_data.name.value;
            var surname = profile_data.surname.value;
            var tipo_documento = profile_data.tipo_documento.value;
            var document = profile_data.document.value;
            var birthday = profile_data.birthday.value;
            var gender = profile_data.gender.value;
            var address = profile_data.address.value;
            var phone_cell = profile_data.phone_cell.value;
            var email = profile_data.email.value;
            var estado_civil = profile_data.estado_civil.value;
            var tipo_sangre = profile_data.tipo_sangre.value;
            var id_pais = profile_data.id_pais.value;
            var id_provincia = profile_data.id_provincia.value;
            var id_ciudad = profile_data.id_ciudad.value;
            var name_profile = profile_data.name_profile.value;

            $('[name="name"]', self.$formDataPersonal).val(name);
            $('[name="surname"]', self.$formDataPersonal).val(surname);
            $('[name="tipo_documento"]', self.$formDataPersonal).attr('_value', tipo_documento);
            $('[name="document"]', self.$formDataPersonal).val(document);
            $('[name="birthday"]', self.$formDataPersonal).val(birthday);
            $('[name="gender"]', self.$formDataPersonal).attr('_value', gender);
            $('[name="address"]', self.$formDataPersonal).val(address);
            $('[name="phone_cell"]', self.$formDataPersonal).val(phone_cell);
            $('[name="email"]', self.$formDataPersonal).val(email);
            $('[name="estado_civil"]', self.$formDataPersonal).attr('_value', estado_civil);
            $('[name="tipo_sangre"]', self.$formDataPersonal).attr('_value', tipo_sangre);
            $('[name="id_pais"]', self.$formDataPersonal).attr('_value', id_pais);
            $('[name="id_provincia"]', self.$formDataPersonal).attr('_value', id_provincia);
            $('[name="id_ciudad"]', self.$formDataPersonal).attr('_value', id_ciudad);
            $('#name_h6').html(name + ' ' + surname + '<small>' + name_profile + '</small>');
            $.uniform.update();
            return;
        }
        profile_data = {
    ...
    {
        var self = this;
        $('[name="name"]', self.$formDataPersonal).prop('readonly', isFormReadOnly);
        $('[name="surname"]', self.$formDataPersonal).prop('readonly', isFormReadOnly);
        $('[name="document"]', self.$formDataPersonal).prop('readonly', isFormReadOnly);
        $('[name="birthday"]', self.$formDataPersonal).prop('readonly', isFormReadOnly);
        $('[name="address"]', self.$formDataPersonal).prop('readonly', isFormReadOnly);
        $('[name="phone_cell"]', self.$formDataPersonal).prop('readonly', isFormReadOnly);
        $('[name="email"]', self.$formDataPersonal).prop('readonly', isFormReadOnly);
        if (isFormReadOnly)
        {
            $('[name="tipo_documento"]', self.$formDataPersonal).prop('disabled', true);
            $('[name="gender"]', self.$formDataPersonal).prop('disabled', true);
            $('[name="estado_civil"]', self.$formDataPersonal).prop('disabled', true);
            $('[name="tipo_sangre"]', self.$formDataPersonal).prop('disabled', true);
            $('[name="id_pais"]', self.$formDataPersonal).prop('disabled', true);
            $('[name="id_provincia"]', self.$formDataPersonal).prop('disabled', true);
            $('[name="id_ciudad"]', self.$formDataPersonal).prop('disabled', true);
        }
        else
        {
            $('[name="tipo_documento"]', self.$formDataPersonal).select2('enable', true);
            $('[name="gender"]', self.$formDataPersonal).select2('enable', true);
            $('[name="estado_civil"]', self.$formDataPersonal).select2('enable', true);
            $('[name="tipo_sangre"]', self.$formDataPersonal).select2('enable', true);
            $('[name="id_pais"]', self.$formDataPersonal).select2('enable', true);
            $('[name="id_provincia"]', self.$formDataPersonal).select2('enable', true);
            $('[name="id_ciudad"]', self.$formDataPersonal).select2('enable', true);
        }
        $.uniform.update();
    },
    // ======================================================================
...
var User_Controller = {
    ...
    formReadOnly: function (isFormReadOnly)
    {
        ...
        $('[name="password_current"]', self.$formDataUser).prop('readonly', isFormReadOnly);
        $('[name="password_new"]', self.$formDataUser).prop('readonly', isFormReadOnly);
        $('[name="password_new_repeat"]', self.$formDataUser).prop('readonly', isFormReadOnly);
        $.uniform.update();
    },
    //====================
    // ERRORS
